...
class AVSR(torch.nn.Module):
    def __init__(self, config, device="cpu"):
        ...
        beam_search_decoder = BeamSearchDecoder(
            self.model, self.token_list, config)
    ...
    def load_model(self, config):
        ...
        assert os.path.isfile(
            model_path), f"model_path: {model_path} does not exist."
        assert os.path.isfile(
            model_conf), f"model_conf: {model_conf} does not exist."
        ...
        if labels_type == "char":
            ...
            file_path = os.path.join(os.path.dirname(
                __file__), "tokens", "unigram5000_units.txt")
            self.token_list = ['<blank>'] + [word.split()[0] for word in open(
                file_path, encoding='utf8').read().splitlines()] + ['<eos>']
    ...
    def infer(self, data):
        with torch.no_grad():
            if isinstance(data, tuple):
                enc_feats = self.model.encode(data[0].to(
